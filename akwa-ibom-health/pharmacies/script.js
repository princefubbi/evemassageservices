  ...
  function runSearch() {
    ...
    document.querySelectorAll('.lga-section').forEach(section => {
      section.style.display = (lga && section.dataset.lga !== lga) ? 'none' : '';
    });
  ...
  function clearFilters() {
    ...
    sidebarType = 'all';
    document.querySelectorAll('.lga-section, .pharmacy-card').forEach(el => el.style.display = '');
    document.getElementById('noResults').classList.remove('visible');
    document.getElementById('visibleCount').textContent = document.querySelectorAll('.pharmacy-card').length;
  }

  function suggestPharmacy() {
    const msg = encodeURIComponent('💊 Pharmacy Suggestion — Akwa Ibom Health Directory\n\nPharmacy name:\nLGA / Location:\nPhone number (if known):\nType (Retail / 24-Hour / Hospital / Community / Compounding):\nNAFDAC registered (Yes/No):');
